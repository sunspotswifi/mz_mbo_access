...
class Settings_Page {
    ...
    private function access_codes(){
        ...
        $return .= "<ul>";
        $return .= "<li><strong>level_2_redirect</strong>: " . __("(url string) URL to redirect users to with membership (service) matching membership_types attribute.", 'mz-mindbody-api')."</li>";
        $return .= "<li><strong>contract_redirect</strong>: " . __("((url string) URL to redirect users to with Contract (membership) matching contract_types attribute. ", 'mz-mindbody-api')."</li>";
        ...
        $return .= "<li><strong>membership_types</strong>: " . __('(string) Comma-separated string of MBO "contract" (membership) types required for access.', 'mz-mindbody-api')."</li>";
        $return .= "<li><strong>contract_types</strong>: " . __('(string) Comma-separated string of MBO "membership" (service) types required for access.', 'mz-mindbody-api')."</li>";
        ...
        $return .= "<li><strong>access_levels</strong>: " . sprintf(__('(int/list) Levels of access required to access content %1$d, %2$d or %3$s', 'mz-mindbody-api'), 1, 2, "\"1, 2\"")."</li>";
        $return .= "</ul>";
        ...
        $return .= sprintf(__('If %1$s %2$s and %3$s are included, the login form will redirect to one of those urls instead of revealing content. Content, if any, between shortcode tags will display when page is reloaded by logged in client.', 'mz-mindbody-api'), '<code>contract_redirect</code>', '<code>level_2_redirect</code>', '<code>denied_redirect</code>');
        return $return;
    }
