...
spl_autoload_register( function( $class_name ) {
	...
	if ( (false === strpos( $class_name, 'MZ_MBO_Access' )) && (false === strpos( $class_name, 'MZ_Mindbody' ))) {
		return;
	}
	...
	if ( $file_parts[0] == 'MZ_Mindbody' ) {
		// We load MZ_Mindbody files from that plugin directory
		$filepath  = trailingslashit( MZ_Mindbody\PLUGIN_NAME_DIR . $namespace );
		$filepath .= $file_name;
	} else {
	...
	if ( file_exists( $filepath ) ) {
		...
		wp_die(
			esc_html( 'The file attempting to be loaded at ' . $filepath . ' does not exist.' . $class_name . ' ' . $namespace )
		);
	}
} );
